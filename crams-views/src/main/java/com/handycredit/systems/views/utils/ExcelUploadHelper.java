...
import com.handycredit.systems.views.UiUtils;
...
public class ExcelUploadHelper implements Serializable {
    ...
    public List<BusinessTransaction> uploadBusinessTransactionsCSVFile(FileUploadEvent event,Business business) {
        try {
            ...
            List<UploadedTransaction> uploadedBusinessTransactions = new CsvToBeanBuilder(reader)
                    .withType(UploadedTransaction.class).build().parse();
            List<BusinessTransaction> clientList = new ArrayList<>();

            uploadedBusinessTransactions.forEach(uploadedBusinessTransaction -> {
                clientList.add(uploadedBusinessTransaction.obtainTransaction(business));
            });
    ...
    public List<BusinessCreditHistory> uploadCreditHistoryCSVFile(FileUploadEvent event, Business business) {
        try {
            UploadedFile uploadedFile = event.getFile();
            ...
            CSVReader reader = new CSVReader(new InputStreamReader(uploadedFile.getInputstream()));
            List<UploadedCreditHistory> uploadedBusinessTransactions = new CsvToBeanBuilder(reader)
                    .withType(UploadedCreditHistory.class).build().parse();
            List<BusinessCreditHistory> clientList = new ArrayList<>();

            uploadedBusinessTransactions.forEach(uploadedBusinessTransaction -> {
                clientList.add(uploadedBusinessTransaction.obtainCreditHistory(business));
            });
    ...
    public static void main(String args[]) throws IOException {
        ...
        try {
            ...
            List<UploadedTransaction> beans = new CsvToBeanBuilder(reader)
                    .withType(UploadedTransaction.class).build().parse();
